...
export class DashboardProducts {
  ...
  editBook(bookId: string): void {
    ...
    if (this.currentBook) {
      ...
      this.editBookPriceAfterDiscount = this.currentBook.priceAfterDiscount?.toString() || '';
      ...
      if (Array.isArray(this.currentBook.subcategory)) {
        if (this.currentBook.subcategory.length > 0) {
          if (typeof this.currentBook.subcategory[0] === 'object' && this.currentBook.subcategory[0] !== null) {
            this.editSubcategoryId = (this.currentBook.subcategory[0] as any)._id || '';
          } else {
        ...
        this.editSubcategoryId = (this.currentBook.subcategory as any)._id || '';
      } else {
  ...
  validateAddForm() {
    ...
    if (!this.addQuantity || isNaN(parseInt(this.addQuantity)) || parseInt(this.addQuantity) < 0) {
      this.formError = 'Valid quantity is required';
      return;
    }

    if (!this.addPrice || isNaN(parseFloat(this.addPrice)) || parseFloat(this.addPrice) < 0) {
    ...
    if (this.addPriceAfterDiscount && (isNaN(parseFloat(this.addPriceAfterDiscount)) || parseFloat(this.addPriceAfterDiscount) < 0)) {
  ...
  validateUpdateForm() {
    ...
    if (!this.editBookQuantity || isNaN(parseInt(this.editBookQuantity)) || parseInt(this.editBookQuantity) < 0) {
      this.formError = 'Valid quantity is required';
      return;
    }

    if (!this.editBookPrice || isNaN(parseFloat(this.editBookPrice)) || parseFloat(this.editBookPrice) < 0) {
      this.formError = 'Valid price is required';
      return;
    }
    ...
    if (this.editBookPriceAfterDiscount && (isNaN(parseFloat(this.editBookPriceAfterDiscount)) || parseFloat(this.editBookPriceAfterDiscount) < 0)) {
      this.formError = 'Valid discount price is required';
      return;
    }
  ...
  async updateBook() {
    ...
    try {
      ...
      if (this.editBookPriceAfterDiscount && this.editBookPriceAfterDiscount.toString().trim()) {
        formData.append('priceAfterDiscount', this.editBookPriceAfterDiscount.toString());
      }

      if (this.editSubcategoryId) {
        // The backend expects an array of subcategory IDs, so we send a JSON stringified array.
        formData.append('subcategory', JSON.stringify([this.editSubcategoryId]));
      }
      ...
      } else if (this.currentBook?.images && this.currentBook.images.length > 0) {
